...
// Origin file: 'c:\projects\graphxgenengine\tests\pipelinecompiler\pipelines\defaultcompute1.cpp'
...
void Create_defaultcompute1 (PipelineTemplateDescriptor& descr)
{
	...
	descr.layout = PipelineLayoutDescriptor::Builder()
			...
			.AddStorageBuffer( "ssb", 16_b, 0_b, EShaderMemoryModel::ReadOnly, 0, 1, EShader::Compute )
			.Finish();
...
layout(binding=0) layout(std430) readonly buffer SSBO {
	layout(offset=0, align=16) vec4  data;
...
void main ()
{
	vec2 uv = (vec2( gl_GlobalInvocationID.xy ) / vec2( (uvec3( 1 ) * gl_NumWorkGroups).xy ));
	vec4 fragColor = vec4( sin( uv.x ), cos( uv.y ), float( 1.0 ), ssb.data.x );
...
0x0000000A, 0x00000007, 0x0004003B, 0x0000001C, 0x0000001D, 0x00000007, 0x00040008, 0x00000001, 0x0000005E, 0x00000000, 0x0004003D, 0x0000000C, 
...
0x00000012, 0x00000019, 0x0003003E, 0x0000000A, 0x0000001A, 0x00040008, 0x00000001, 0x0000005F, 0x00000000, 0x00050041, 0x0000001F, 0x00000020, 
...
0x00000001, 0x00000060, 0x00000000, 0x0004003D, 0x00000030, 0x00000033, 0x00000032, 0x0004003D, 0x0000000C, 0x00000034, 0x0000000E, 0x0007004F, 
	...
	/*1*/__global read_only struct SSBO* ssb)
{
	...
	float2 H_xulukk = convert_float2( H_fjkh5n );
	int2 H_k5ommu = convert_int3( ((uint3)(get_num_groups(0), get_num_groups(1), get_num_groups(2))) ).xy;
	float2 H_nnu7tn1 = convert_float2( H_k5ommu );
	float2 uv = (H_xulukk / H_nnu7tn1);
	float fragColor = sin( uv.x );
	float fragColor1 = cos( uv.y );
	float4 H_imjy6u1 = ((float4)( fragColor, fragColor1, C_1d0, ssb->data.x ));
