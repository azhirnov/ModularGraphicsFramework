...
bool PApp::_Test_VecSwizzle ()
{
	const BytesU	buf_size = SizeOf< Pipelines::VecSwizzle_SSBO >;
	...
	Pipelines::Create_vecswizzle( OUT pt_ci.descr );
	...
	auto* st = Cast<const Pipelines::VecSwizzle_SSBO *>( read_cmd->result->ptr() );

	CHECK_ERR( st->results[0] == 88 );
	CHECK_ERR( st->results[1] == 16 );
	CHECK_ERR( st->results[2] == 110 );
	CHECK_ERR( st->results[3] == 7 );
	CHECK_ERR( st->results[4] == 9 );
	CHECK_ERR( st->results[5] == 12 );
	CHECK_ERR( st->results[6] == 10 );
	//CHECK_ERR( st->results[7] == 4373 );

	LOG( "VecSwizzle - OK", ELog::Info );
	return true;
}
