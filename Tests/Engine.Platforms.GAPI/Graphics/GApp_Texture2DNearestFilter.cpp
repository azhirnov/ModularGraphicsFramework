...
bool GApp::_Test_Texture2DNearestFilter ()
{
	...
	CHECK_ERR( factory->Create(
					...
					gpuThread->GlobalSystems(),
					CreateInfo::GpuImage{
						ImageDescriptor{ EImage::Tex2D, uint4(src_img_dim), EPixelFormat::RGBA8_UNorm, EImageUsage::TransferSrc },
	...
	CHECK_ERR( factory->Create(
					...
					gpuThread->GlobalSystems(),
					CreateInfo::GpuImage{
						ImageDescriptor{ EImage::Tex2D, uint4(dst_img_dim), EPixelFormat::RGBA8_UNorm, EImageUsage::TransferDst },
						EGpuMemory::CoherentWithCPU },
	...
	CHECK_ERR( factory->Create(
					...
					gpuThread->GlobalSystems(),
					CreateInfo::GpuImage{
						ImageDescriptor{ EImage::Tex2D, uint4(src_img_dim), EPixelFormat::RGBA8_UNorm, EImageUsage::TransferDst | EImageUsage::Sampled },
	...
	CHECK_ERR( factory->Create(
					...
					gpuThread->GlobalSystems(),
					CreateInfo::GpuImage{
						ImageDescriptor{ EImage::Tex2D, uint4(dst_img_dim), EPixelFormat::RGBA8_UNorm, EImageUsage::TransferDst | EImageUsage::TransferSrc | EImageUsage::Storage },
						EGpuMemory::LocalInGPU,
						EMemoryAccess::GpuReadWrite },
					OUT dst_image ) );
	...
	CHECK_ERR( factory->Create(
					...
					CreateInfo::GpuSampler{
						...
						.SetBorderColor( ESamplerBorderColor::Float | ESamplerBorderColor::White )
						.Finish()
					},
					OUT sampler ) );
	...
	Message< GpuMsg::CreateComputePipeline >	cppl_ctor{ computeIDs.pipeline, gpuThread };
	...
	Message< GpuMsg::WriteToImageMemory >	write_cmd{ image_data, uint3(), uint3(src_img_dim), req_src_layout->result->rowPitch };
	...
	Message< GpuMsg::ReadFromImageMemory >	read_cmd{ dst_data, uint3(), uint3(dst_img_dim), req_dst_layout->result->rowPitch };
	...
	{
		uint			y1		= y < offset.y ? 0 : y - offset.y;
						y1		= y1 * 2 + (y >= dst_img_dim.y/2 ? 1 : 0);
		...
		{
			uint	x1  = x < offset.x ? 0 : x - offset.x;
					x1  = x1 * 2 + (x >= dst_img_dim.x/2 ? 1 : 0);
			Pixel	src = src_row[x1];
			Pixel	dst = dst_row[x];

			if ( x < offset.x or y < offset.y ) {
				// border
				CHECK(All( white == dst ));
			} else {
				CHECK(All( src == dst ));
	...
	LOG( "Texture2DNearestFilter - OK", ELog::Info );
	return true;
}
