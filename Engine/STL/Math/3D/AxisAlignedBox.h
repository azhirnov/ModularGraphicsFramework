...
*/
	template <typename T>
	ND_ inline bool  AABBox<T>::operator == (const AABBox<T> &right) const
	{
...
*/
	template <typename T>
	ND_ inline AABBox<T>  AABBox<T>::operator + (const Vec<T,3> &right) const
	{
		return AABBox<T>(*this) += right;
	}

	template <typename T>
	ND_ inline AABBox<T>  AABBox<T>::operator + (const AABBox<T> &right) const
	{
		return AABBox<T>(*this) += right;
	}
...
*
	template <typename T>
	ND_ inline AABBox<T>  AABBox<T>::operator * (const Matrix<T,4,4> &right) const
	{
		AABBox<T>	ret(*this);
	...
	template <typename T>
	ND_ inline AABBox<T>  AABBox<T>::operator * (const Matrix<T,3,3> &right) const
	{
		AABBox<T>	ret(*this);
		ret.Transform( right );
		return ret;
...
*/
	template <typename T>
	ND_ inline T  AABBox<T>::Volume () const
	{
...
*/
	template <typename T>
	ND_ inline T  AABBox<T>::MinRadius () const
	{
...
*/
	template <typename T>
	ND_ inline T  AABBox<T>::MaxRadius () const
	{
		const Vec<T,3>	side		= Extent();
...
*/
	template <typename T>
	ND_ inline bool  AABBox<T>::IsInside (const Vec<T,3> &point) const
	{
...
*/
	template <typename T>
	ND_ inline bool  AABBox<T>::IsFullInside (const Vec<T,3> &point) const
	{
		return All( point > _min ) and All( point < _max );
	}
...
*/
	template <typename T>
	ND_ inline bool  AABBox<T>::IsInside (const AABBox<T> &box) const
	{
...
*/
	template <typename T>
	ND_ inline bool  AABBox<T>::IsFullInside (const AABBox<T> &box) const
	{
		return All( box._min > _min ) and All( box._max < _max );
	}
...
*/
	template <typename T>
	ND_ inline bool  AABBox<T>::Intersect (const AABBox<T> &box) const
	{
...
*/
	template <typename T>
	ND_ inline AABBox<T>  AABBox<T>::Intersection (const AABBox<T> &box) const
	{
		AABBox<T>	ret( *this );
...
*/
	template <typename T>
	ND_ inline Vec<T,3>  AABBox<T>::GetCorner (_Corner_t corner) const
	{
		switch ( corner )
		{
...
*/
	template <typename T>
	template <typename T2>
	ND_ inline AABBox<T2>  AABBox<T>::Convert () const
	{
