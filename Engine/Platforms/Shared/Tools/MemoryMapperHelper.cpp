...
=================================================
*/
	bool MemoryMapperHelper::ReadImage (OUT BytesUL &readn, OUT BinArrayRef result, const uint4 &dstDimension, BytesUL dstRowPitch, BytesUL dstSlicePitch,
										const uint4 &srcOffset, const uint4 &srcDimension, BytesUL srcMemOffset, BytesUL srcMemSize, BytesUL srcRowPitch, BytesUL srcSlicePitch) const
	{
		CHECK_ERR( IsMapped() );
		CHECK_ERR( _mappingAccess[EMemoryAccess::CpuRead] );
		CHECK_ERR( dstDimension.w == 1 and All( dstDimension.xyz() > uint3(0) ) );
		CHECK_ERR( All ( srcDimension + srcOffset <= srcDimension ) );
...
=================================================
*/
	bool MemoryMapperHelper::WriteImage (OUT BytesUL &written, BinArrayCRef data, const uint4 &srcDimension, BytesUL srcRowPitch, BytesUL srcSlicePitch,
										 const uint4 &dstOffset, const uint4 &dstDimension, BytesUL dstMemOffset, BytesUL dstMemSize, BytesUL dstRowPitch, BytesUL dstSlicePitch)
	{
		CHECK_ERR( IsMapped() );
		CHECK_ERR( _mappingAccess[EMemoryAccess::CpuWrite] );
		CHECK_ERR( srcDimension.w == 1 and All( srcDimension.xyz() > uint3(0) ) );
		CHECK_ERR( All ( srcDimension + dstOffset <= dstDimension ) );
		CHECK_ERR( dstMemOffset + dstMemSize <= _size );

		uint3	coord;
		ubyte*	dst_end	= _ptr + (dstMemOffset + dstMemSize);
		BitsUL	bpp		= BitsUL(dstRowPitch) / dstDimension.x;

		for (; coord.z < srcDimension.z; ++coord.z)
		{
			for (; coord.y < srcDimension.y; ++coord.y)
			{
				BytesUL			src_size = (coord.z * srcSlicePitch + coord.y + srcRowPitch * coord.y);
