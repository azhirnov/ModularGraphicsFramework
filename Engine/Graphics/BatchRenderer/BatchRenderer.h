...
namespace GraphicsMsg
{
	...
	// variables
		...
		ModulePtr			cmdBuilder;		// optional, if defined then commands will be writed to current command buffer, 'cmdBuffer' will be undefined
		Out< ModulePtr >	cmdBuffer;
		
	// methods
		FlushBatchRenderer (const ModulePtr &framebuffer, const GXMath::RectU &viewport, const ModulePtr &cmdBuilder = null) :
