...
namespace Graphics
{
	...
	private:
		...
		CommandBuffers_t			_cmdBuffers;
		//ModulePtr					_managedMemory;
...
*/
		...
		_descr( ci )
	{
		...
		_SubscribeOnMsg( this, &BatchRenderer::_DeviceBeforeDestroy );

		CHECK( _ValidateMsgSubscriptions() );

		_AttachSelfToManager( null, GraphicsThreadModuleID, true );
...
*/
	bool BatchRenderer::_Link (const Message< ModuleMsg::Link > &msg)
	{
		...
		ModulePtr	gthread;
		CHECK_LINKING( gthread = GlobalSystems()->Get< ParallelThread >()->GetModuleByMsg< GThreadMsgList_t >() );
...
*/
	bool BatchRenderer::_Compose (const Message< ModuleMsg::Compose > &)
	{
		...
		auto	factory = GlobalSystems()->Get< ModulesFactory >();

		CHECK_ERR( factory->Create(
						_moduleIDs.commandBuilder,
						GlobalSystems(),
						CreateInfo::GpuCommandBuilder{},
						OUT _cmdBuilder ) );

		_cmdBuffers.Reserve( 8 );
		_batches.Reserve( 16 );
...
*/
	bool BatchRenderer::_BeginBatchRenderer (const Message< GraphicsMsg::BeginBatchRenderer > &msg)
	{
		...
		{
			ModulePtr	dev;
			CHECK_ERR(( dev = GlobalSystems()->Get< ParallelThread >()->GetModuleByMsgEvent< DeviceMsgList_t, DeviceEventList_t >() ));
...
*/
	bool BatchRenderer::_FlushBatchRenderer (const Message< GraphicsMsg::FlushBatchRenderer > &msg)
	{
		...
		// build commands
		{
			const bool	external_builder	= msg->cmdBuilder.IsNotNull();
			auto		builder				= external_builder ? msg->cmdBuilder : _cmdBuilder;
			uint		idx_offset			= 0;

			if ( not external_builder )
				builder->Send< GpuMsg::CmdBegin >({ true });
			...
			if ( not external_builder )
			{
				...
				msg->cmdBuffer.Set( cmd_end->cmdBuffer.Get() );
			}
			else
			{
				msg->cmdBuffer.Set( null );
			}
		}
...
//-----------------------------------------------------------------------------
	...
	ModulePtr GraphicsContext::_CreateBatchRenderer (GlobalSystemsRef gs, const CreateInfo::BatchRenderer &ci)
	{
