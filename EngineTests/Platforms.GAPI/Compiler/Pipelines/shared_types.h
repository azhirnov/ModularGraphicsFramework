...
// Packing: Std430
struct Include_SSBO final
{
	float  results[2];    // offset: 0, align: 4

	Include_SSBO ()
	{
		STATIC_ASSERT( sizeof(Include_SSBO) == 8 );
		STATIC_ASSERT( (offsetof( Include_SSBO, results ) == 0) and (sizeof( results ) == 8) );
	}
};
