...
DECL_PIPELINE( Skyline, ShadertoyTemplate,
{
	const String path = FileAddress::BuildPath( FileAddress::GetPath( __FILE__ ), "../../Engine/PipelineCompiler/glsl/" );

	shaders.fragment.Load( path + "Common/Defines.glsl" )
					.Load( path + "Common/Cmp.glsl" )
					.Load( path + "Math/MathDef.glsl" )
					.Load( path + "Math/Math.glsl" )
					.Load( path + "Math/Utils.glsl" )
					.Load( path + "Math/Quaternion.glsl" )
					.Load( path + "Math3D/Line3.glsl" )
					.Load( path + "Math3D/Ray.glsl" )
					.Load( __FILE__ );
})
#endif
...
vec3 RayTrace (const vec2 fragCoord, const Ray ray)
{
	...
	vec3 rayVec;
	{
		...
		rayVec = ray.dir; //normalize(worldPix - camPos);
	}
...
{
	Ray	ray = Ray_From( iCameraFrustum[0], iCameraFrustum[1], iCameraFrustum[2], iCameraFrustum[3],
						iCameraPos, 10.0, fragCoord / iResolution.xy );
