...
static void Test_AlignedImageSize ()
{
	...
	size = GXImageUtils::AlignedDataSize( uint3( 2, 2, 1 ), 8_bit.ToBytes(), 1_b, 1_b );
	TEST( size == 2*2 );

	size = GXImageUtils::AlignedDataSize( uint3( 2, 2, 1 ), 8_bit.ToBytes(), 4_b, 1_b );
	TEST( size == 4*2 );
	
	size = GXImageUtils::AlignedDataSize( uint3( 4, 2, 1 ), 8_bit.ToBytes(), 4_b, 1_b );
	TEST( size == 4*2 );
	
	size = GXImageUtils::AlignedDataSize( uint3( 2, 4, 1 ), 8_bit.ToBytes(), 4_b, 1_b );
	TEST( size == 4*4 );
	
	size = GXImageUtils::AlignedDataSize( uint3( 3, 2, 1 ), (8_bit * 3).ToBytes(), 4_b, 1_b );
	TEST( size == 4*2*3 );
