...
#include "Core/STL/CompileTime/TemplateMath.h"
...
*/
	template <typename R, typename T>
	ND_ forceinline constexpr R  DynCast (T *ptr)
	{
		return dynamic_cast<R>( ptr );
	}

	template <typename R, typename T>
	ND_ forceinline constexpr const R  DynCast (const T *ptr)
	{
		return dynamic_cast<const R>( ptr );
	}
