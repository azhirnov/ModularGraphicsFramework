...
*/
										  ...
										  OUT usize &leftLineNumber, OUT usize &rightLineNumber)
	{
		...
		const auto	LeftValid	= LAMBDA( &l_pos, &left  ) ()	{{ return (l_pos < left.Length()); }};
		const auto	RightValid	= LAMBDA( &r_pos, &right ) ()	{{ return (r_pos < right.Length()); }};

		const auto	IsEmptyLine	= LAMBDA() (StringCRef str)
		{{
			for (auto& c : str) {
				if ( c != '\n' and c != '\r' and c != ' ' and c != '\t' )
					return false;
		...
		for (; LeftValid() and RightValid(); )
		{
			// read left line
			do {
				StringParser::ReadLineToEnd( left, INOUT l_pos, OUT leftLine );
				++leftLineNumber;
			}
			while ( IsEmptyLine( leftLine ) and LeftValid() );

			// read right line
			do {
				StringParser::ReadLineToEnd( right, INOUT r_pos, OUT rightLine );
				++rightLineNumber;
			}
			while ( IsEmptyLine( rightLine ) and RightValid() );
